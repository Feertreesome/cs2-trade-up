...
exports.fetchListingTotalCount = exports.searchByCollection = exports.fetchCollectionTags = exports.searchByRarity = exports.RARITY_TO_TAG = exports.getPriceUSD = exports.steamGet = void 0;
...
const steamGet = async (url, requestConfig) => enqueue(async () => {
    ...
    for (let attempt = 0; attempt < maxAttempts; attempt++) {
        try {
            ...
            if (status === 429) {
                bumpRate();
                cooldownUntilTs = Date.now() + 15000; // общий «отдых»
            }
 ...
 */
const steamGetData = async (url, requestConfig) => {
    const response = await (0, exports.steamGet)(url, requestConfig);
...
const getPriceUSD = async (marketHashName) => {
    ...
    try {
        ...
        const payload = await steamGetData(`${PRICE_URL}?${params.toString()}`);
        if (!payload?.success) {
...
const searchByRarity = async ({ rarity, start = 0, count = 30, normalOnly = true, }) => {
    ...
    if (cached)
        return cached;
    // ВАЖНО: типизируем data
    const payload = await steamGetData(url);
